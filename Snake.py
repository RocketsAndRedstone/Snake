...
apple.penup()
apple.shape("circle")
apple.color("Red")#creates the snake that you are controling, the apple to be consumed, and the playing board
def main():
    playing = True
    tailCords = []
    colisionArea = 6
    ...
    while playing == True:
        ...
        if (pencil.xcor() - colisionArea <= applePosition[0]) and (pencil.xcor() + colisionArea >= applePosition[0]) and (pencil.ycor() + colisionArea >= applePosition[1]) and (pencil.ycor() - colisionArea <= applePosition[1]):
            ...
            score += 1
            tailCords.append(pencil.xcor())#used for the colision detection between the head and tail for the snake
            ...
            if timesMoved == 0:
                ...
                pencil.fd(5)
                for i in range(2):
                    del tailCords[0]#removes the tail's cordanents for the segment that was just deleted on screen
                pencil.stamp()
        ...
        turtle.listen()#gets the WASD input to controll the snake
        for i in range(int((len(tailCords) / 2))):
            if (pencil.xcor() - colisionArea1 <= tailCords[i]) and (pencil.xcor() + colisionArea1 >= tailCords[i] and pencil.ycor() - colisionArea1 <= tailCords[i + 1]) and (pencil.ycor() + colisionArea1 >= tailCords[i + 1]):
                deathScreen(score)#Detects if the snake head has colided with any part of the tail, if so, it displays the amount of apples eaten
                playing = False
        if pencil.xcor()>= 95 or pencil.xcor()<= -95 or pencil.ycor()<= -95 or pencil.ycor()>= 95:#detects if the head of the snake has colided with any of the walls
            deathScreen(score)
            playing = False
                  
def placeApple():
    apple.clearstamps(1)#removes the privious apple so that it is fully consumed
